...
| Velcro straps | 2 | One for calf, one for thigh: component placement |
...
Accelerometers react extremely fast but are highly sensitive to high-frequency noise from walking vibrations. Conversely, gyroscopes measure angular velocity accurately in the short term, but integrating this data over time introduces numerical errors, causing the calculated angle to drift. To compensate for these mutual limitations, the system implements a customized Madgwick AHRS (Attitude and Heading Reference System) filter.

Whereas simple one-dimensional filters fail under minor mounting deviations on a flexible Velcro strap due to *cross-axis coupling* (where joint rotation leaks across multiple sensor axes), the Madgwick filter solves this spatially by calculating absolute orientations in 4D Quaternions ($Q = [W, X, Y, Z]$). The relative knee joint angle is calculated in three distinct steps:

* **Absolute Orientation:** The filters ingest the raw 6-DoF inertial data to calculate the absolute 3D position of the thigh segment ($Q_{\text{thigh}}$) and the calf segment ($Q_{\text{calf}}$) relative to the global gravity vector.
* **Relative Spatial Angle:** The relative orientation between the two independent leg segments ($Q_{\text{diff}}$) is calculated by mathematically subtracting the thigh's spatial orientation from the calf's orientation via quaternion multiplication with the mathematical inverse:
$$Q_{\text{diff}} = Q_{\text{thigh}}^{-1} \otimes Q_{\text{calf}}$$
* **Anatomical Alignment Compensation:** The resulting quaternion $Q_{\text{diff}}$ purely isolates the structural movement within the joint itself. Any initial misalignment, twist, or human error introduced while strapping the sensors onto the leg is mathematically neutralized, rendering the system entirely free from *gimbal lock*.
